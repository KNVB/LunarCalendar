 ...
 * @author Roy Tsang
 */
public class LunarDateObjectFactory 
{
	...
	public LunarDateTime getLunarDateTime(LocalDateTime inLocalDateTimeObj)
	{
		...
		if(inLocalDateTimeObj.getMonthValue()<Month.MARCH.getValue())
		{
			result.yearPillar=getCyclical(inYear-1900+36-1);
			result.animalOfYear=getAnimalOfYear(inYear-1);
		}
		else
		{
			 result.yearPillar=getCyclical(inYear-1900+36);
			 result.animalOfYear=getAnimalOfYear(inYear);
		}
		
		////////月柱 1900年1月小寒以前為 丙子月(60進制12)
		firstNode = sTerm(inYear,(inMonth-1)*2); //傳回當月「節」為幾日開始
		
		//依節氣月柱, 以「節」為界
		if(inDate>=firstNode) 
			result.monthPillar  = getCyclical((inYear-1900)*12+inMonth+12);
		else
			result.monthPillar = getCyclical((inYear-1900)*12+inMonth+11);
		...
		//日柱
		result.datePillar= getCyclical((int)dayCyclical+inDate);
		
		//時柱
		result.hourPillar=getChineseHourName(inLocalDateTimeObj.getHour());
	...
	public MonthlyCalendar getMonthlyCalendar(int year,int month)
	{
		...
		Hashtable<Integer,MyDateTime>myCalendarList=new Hashtable<Integer,MyDateTime>();
		
		for (i=1;i<=mc.length;i++)
		{
			
			sDObj = LocalDateTime.of(year,month,i,0,0,0);
			lDObj=getLunarDateTime(sDObj);
	...
	/**
	 * 傳回該年的復活節LocalDateTime物件(春分後第一次滿月週後的第一主日)<br>
	 * It returns a LocalDateTime object which devote the date of easter of given year
	 * @param y 年份
	 * @return 傳回該年復活節LocalDateTime物件
	 */
	public LocalDateTime getEasterDateByYear(int y)
	{
		...
		LunarDateTime lDayTerm2 = getLunarDateTime(dayTerm2); //取得取得春分農曆
 ...
 *===========================================*/
	 ...
	 */
	private void processLunarYearHoliday(Hashtable<Integer,MyDateTime>myCalendarList,Hashtable<Integer,String>lunarHolidayDates) 
	{
		MyDateTime m;
	 ...
	 * @param inDate
	 */
	private void processHoliday(Hashtable<Integer, MyDateTime> myCalendarList, String festivalInfo,int inDate) 
	{
		MyDateTime m1=myCalendarList.get(inDate);
	 ...
	 * @param inDate
	 */
	private void holidayCompensation(Hashtable<Integer, MyDateTime> myCalendarList, String festivalInfo,int inDate) 
	{
		MyDateTime m1,m2; 
	 ...
	 * @param date 當日的日期
	 */
	private void setFestivalInfo(Hashtable<Integer,MyDateTime>myCalendarList,String festivalInfo,int date)
	{
		MyDateTime m=myCalendarList.remove(date);
	 ...
	 * @param date 當日的日期
	 */
	private void setHoliday(Hashtable<Integer,MyDateTime>myCalendarList,String festivalInfo,int date)
	{
		MyDateTime m=myCalendarList.remove(date);
	 ...
	 * @param inDate
	 */
	private void processEasterHoliday(Hashtable<Integer,MyDateTime>myCalendarList,int year,int month) 
	{
